...
def begin_graphics(width=640, height=480, color=formatColor(0, 0, 0), title=None):
    ...
    try:
      ...
      polygon(corners, color, fillColor=color, filled=True, smoothed=False, behind=True)
      _canvas.update()
    except:
      _root_window = None
      raise
...
def polygon(coords, outlineColor, fillColor=None, filled=1, smoothed=1, behind=False, width=1):
  c = []
  for coord in coords:
    c.append(coord[0])
    c.append(coord[1])
  ...
  if  behind:    _canvas.tag_lower(poly, 2) # Just above the background
  return poly
