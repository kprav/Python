...
def readCommand( argv ):
  ...
  args = dict()
  # Fix the random seed
  if options.fixRandomSeed: random.seed('cs561')  
  # Choose a layout
...
def runGames( layout, pacman, ghosts, display, numGames, record, numQuiet = 0, numIgnore = 0 ):
  import __main__
  ...
  for i in range( numGames ):
    # Supress GUI if < numQuiet
    beQuiet = i < numQuiet
    if beQuiet:
        import textDisplay        
        gameDisplay = textDisplay.NoGraphics()
        rules.quiet = True
    else:
        gameDisplay = display
        rules.quiet = False    
    ...
    game.run()
    if i >= numIgnore: games.append(game)
    if record:
      import time, cPickle
  ...
  if numGames > 1:
    ...
    print 'Win Rate:     ', wins.count(True) / float(len(wins))
